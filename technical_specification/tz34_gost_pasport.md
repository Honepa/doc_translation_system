...
## 3.2.1. Алгоритм использования системы

Аутентифицированный пользователь системы обязан следовать следующему алгоритму работы с системой:

1. Перед началом работы с системой пользователь должен выбрать режим:

1.1. Режим добавления нового документа в базу:
	
1.1.1. Для добавления документа в базу, пользовать должен иметь отсканированный документ;
	
1.1.2. Далее, в веб-интерфейсе пользователь должен нажать на соответсвующую кнопку для загрузки новых данных (в соответствии с приложением);
	
1.1.3. После, для пользователя открывается форма для загрузки данных;
	
1.1.4. В открывшийся форме, пользователь заполняет соответсвующие пункты, такие как выбор файла отсканированного документа на локальном компьютерной системе пользователя, название и т.д. (в соответсвие с приложением);
	
1.1.5. После заполнения всех всех полей, пользователь нажимает на соответсвующую кнопку сохранения нового документа в базе, закрывается форма;
	
1.2. Режим работы с загруженными документами из базы:
	
1.2.1. Для работы с уже загруженными данными пользователь нажимает на соответсвующую кнопку в веб-интерфейсе;
	
1.2.2. Пользователь, с помощью формы поиска, вводит ключевые слова документа и иему выводится отсканированный документ для подтверждения правильности найденного документа;
	
1.2.3. После нахождения нужного документа, пользователю открывается интерфейс работы с документом (в соответсвие с приложением);
	
1.2.4. Проделав необхожимые операции пользователь сохраняет изменения и выходит из формы редактирования документа и(или) выгружает готовый документ в формате "docx";
