...
## 3.2.2. Алгоритм работы пользователя
...
1.1.1. пользователю предлагется выбрать тип документа, выбрав его из предложенной формы выбора типа документа;
	
1.1.2. после открывается форма редактирования документа, где слева находится отсканированный документ, а справа шаблон заполнения документа переводчиком. Так же есть возможность довать данные из подсистемы распознавания текстов;

1.1.3. пользователь может настройть режим доступа к документу по ссылке (чтение, редактирование, печать);
 
1.1.4. после оканчания работ по редактирования докуманта, пользователь сохраняет документ в базе и при необходимости выгружает получившийся шаблон в формате "Office Open XML" ("docx") (можно вывести на печать);
	
1.2. режим работы с загруженными документами из базы:
	
1.2.1. пользователь с помощью формы поиска находит нужный элемент в базе;
	
1.2.2. пользователь проделывает операции п. 1.1.2 - 1.1.4.
